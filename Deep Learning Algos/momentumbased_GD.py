...
# Train the model
def train_model_momentum(model, criterion, optimizer, num_epochs=10):
    train_losses = []
    test_losses = []
...
# Initialize the model, loss function, and optimizer with momentum
model_momentum = NeuralNetwork()
criterion_momentum = nn.BCELoss()
optimizer_momentum = optim.SGD(model_momentum.parameters(), lr=0.01, momentum=0.9)

# Train the model with momentum-based gradient descent
train_losses_momentum, test_losses_momentum = train_model_momentum(model_momentum, criterion_momentum, optimizer_momentum, num_epochs=10)

# Plot the loss curves
plt.plot(train_losses_momentum, label='Training Loss')
plt.plot(test_losses_momentum, label='Test Loss')
plt.title('Training and Test Loss (Momentum-based)')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
