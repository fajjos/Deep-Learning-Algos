...
# Train the model
def train_model_sgd(model, criterion, optimizer, num_epochs=10):
    train_losses = []
    test_losses = []
...
train_losses_sgd, test_losses_sgd = train_model_sgd(model_sgd, criterion_sgd, optimizer_sgd, num_epochs=10)

# Plot the loss curves
plt.plot(train_losses_sgd, label='Training Loss')
plt.plot(test_losses_sgd, label='Test Loss')
plt.title('Training and Test Loss (SGD)')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
