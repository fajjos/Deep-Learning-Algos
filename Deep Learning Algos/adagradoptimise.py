...
# Train the model
def train_model_adagrad(model, criterion, optimizer, num_epochs=10):
    train_losses = []
    test_losses = []
...
# Initialize the model, loss function, and optimizer with Adagrad optimizer
model_adagrad = NeuralNetwork()
criterion_adagrad = nn.BCELoss()
optimizer_adagrad = optim.Adagrad(model_adagrad.parameters(), lr=0.01)

# Train the model with Adagrad optimizer
train_losses_adagrad, test_losses_adagrad = train_model_adagrad(model_adagrad, criterion_adagrad, optimizer_adagrad, num_epochs=10)

# Plot the loss curves
plt.plot(train_losses_adagrad, label='Training Loss')
plt.plot(test_losses_adagrad, label='Test Loss')
plt.title('Training and Test Loss (Adagrad Optimizer)')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
